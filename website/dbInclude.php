<?php 
	...
	function dbsanitize(mysqli $dbLink, $data)
	{
		if ( empty($data) )
		{	return 'NULL';}
		else if ( is_numeric($data) )
		{	return mysqli_real_escape_string($dbLink, urldecode($data)); }
		else
		{  return "'" . mysqli_real_escape_string($dbLink, urldecode($data)) . "'"; }
	}
	...
	function fetchAPI(mysqli $dbLink, $SiteRequest)
	{
		...
		$result = c_mysqli_call($dbLink, 'newLoadBalancerQuery', "'".mysqli_real_escape_string($dbLink,$SiteRequest) . "'");
		if($result) {
			foreach($result as $_row) {
				return $_row['id'];
			}
	...
	function better_error_handler($errno, $errstr, $errfile, $errline, $errcontext) {
		if (error_reporting() !== 0) {
			//echo "[DI:" . $errfile . "," . $errline . "] " . $errstr . "<br>";
		}
